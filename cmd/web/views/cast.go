...
func CastTableView(cast []*models.CastMember, sketchID int, baseImgUrl string) CastTable {
	...
	for _, c := range cast {
		...
		if c.ProfileImg == nil && c.Actor != nil {
			row.ImageUrl = fmt.Sprintf("%s/person/medium/%s", baseImgUrl, safeDeref(c.Actor.ProfileImg))
		} else {
			row.ImageUrl = fmt.Sprintf("%s/cast/profile/medium/%s", baseImgUrl, safeDeref(c.ProfileImg))
		}
...
func DetermineCastImageUrl(cm *models.CastMember, size, baseImgUrl string) string {
	...
	if safeDeref(cm.ProfileImg) != "" {
		...
		if cm.Actor.ID != nil && cm.Actor.Slug != nil {
			if cm.Actor.ProfileImg != nil {
				img = fmt.Sprintf(
					"%s/person/%s/%s",
					baseImgUrl,
					size,
