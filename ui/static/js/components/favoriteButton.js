...
export class FavoriteButton extends HTMLElement {
  ...
  async like() {
    ...
    if (!response.ok) {
      this.showPopup(this.favButton, 'Error favoriting video...');
      return;
    }
  ...
  showPopup(element, message) {
    ...
    popup.className = 'absolute -top-6 left-1/2 -translate-x-1/2 whitespace-nowrap transform bg-slate-900 text-white text-xs px-2 py-1 rounded opacity-0 transition-opacity duration-300';
